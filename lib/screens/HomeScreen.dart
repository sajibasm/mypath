...
// ✅ Background write isolate function
Future<void> writeSensorData(SensorData data) async {
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  DateTime? _lastUpdate; // ✅ For throttling UI updates
  Timer? _cameraUpdateTimer; // ✅ For debouncing camera animation
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _loadUserInfoAndSession(); // ✅ Delayed execution to reduce UI jank
    });
  }
  ...
  Future<void> _startDataCollection() async {
    ...
    _cameraUpdateTimer = Timer(const Duration(milliseconds: 500), () {
      if (_isMapReady && _mapController != null) {
        _mapController!.animateCamera(CameraUpdate.newLatLngZoom(point, 18.5));
      }
    });

    _locationSub = _locationService.locationStream.listen((loc) async {
      final now = DateTime.now();
      if (_lastUpdate != null && now.difference(_lastUpdate!) < const Duration(seconds: 1)) return;
      _lastUpdate = now;
      ...
      if (_routePoints.isEmpty ||
        ...
        _cameraUpdateTimer?.cancel();
        _cameraUpdateTimer = Timer(const Duration(milliseconds: 1000), () {
          if (_isMapReady && _mapController != null) {
